...
S_KERNEL=${6} # e.g. 2.55
PERM=${7} # number of permutation (which random order of runs shall be used?)
...
--motion ${work_dir}/*.mat \
--mre-dir ${MRE_DIR} \
--left ${INFOLDER}/sub-${SUB}/ses-${SES}/anat/sub-${SUB}_ses-${SES}_hemi-L_space-MNI_mesh-fsLR32k_midthickness.surf.gii \
--right ${INFOLDER}/sub-${SUB}/ses-${SES}/anat/sub-${SUB}_ses-${SES}_hemi-R_space-MNI_mesh-fsLR32k_midthickness.surf.gii \
--additional-mask ${work_dir}/masks/sub-${SUB}_ses-${SES}_mask_groundtruth_${MAXMIN}min.txt \
--wb-command ${WB_CMD} --fd-threshold ${FD} --smoothing-kernel ${S_KERNEL} \
...
D_ONE=${work_dir}/half1/${MIN}min/sub-${SUB}_ses-${SES}_task-${TASK}_bold_shuffled_timeseries_SMOOTHED_${S_KERNEL}.dtseries.nii_all_frames_at_FD_${FD}.dconn.nii
D_TWO=${work_dir}/groundtruth/sub-${SUB}_ses-${SES}_task-${TASK}_bold_shuffled_timeseries_SMOOTHED_${S_KERNEL}.dtseries.nii_all_frames_at_FD_${FD}.dconn.nii
...
GIFTI_C='/path/to/my/utilities/gifti/'

mkdir ${work_dir}/rel_val/; 
matlab -r "addpath('/path/to/my/code'); CalculateDconntoDconnCorrelationIndividualSeedsNoDistMat('DconnShort','${D_ONE}','DconnGroundTruth','${D_TWO}', 'OutputDirectory','${OUTDIR}', 'OutputName','${OUTNAME}', 'wb_command','${WB_CMD}', 'CIFTI_path','${CIFI_C}', 'GIFTI_path','${GIFI_C}')"
